...
class DAN(object):
    def __init__(self, image_size, batch_size=32, c_dim=1, f_dim=32, d=64, num_class=3, is_train=True, r=1, a=None):
    ...
    def build_model(self):
        ...
        for c in range(len(Pyramid_features)+2, 2, -1):
            ...
            if not self.is_train:
                ...
                self.acc_class3['class3_acc_classifier%d'%c] = class_acc[2]
        ...
        if self.is_train:
            ...
            self.class3_acc_sum = tf.summary.scalar("class3_acc", self.avg_acc_class3)
        else:
            self.C5_maps = maps[0]
            self.C4_maps = maps[1]
            self.C3_maps = maps[2]
    ...
    def classifiers(self, features, target, name='Classifier', train=True, reuse=tf.AUTO_REUSE):
        ...
        CAM= tf.nn.dropout(class_activation_maps, keep_prob=self.dr_rate, name=name+'CAM_dr')
        ...
        if train:
            target = get_3labels_batch(target)
