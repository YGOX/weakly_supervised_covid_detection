...
def train_and_predict(lr, batch_size, image_size, num_iter, r, fold_id):
    ...
    train_neumo = np.load('captrain_lists.npy', allow_pickle=True)[fold_id]
    ...
    valid_neumo = np.load('capvalid_lists.npy', allow_pickle=True)[fold_id]
    trainfiles = train_normal + train_covid + train_neumo
    train_labels= np.concatenate((np.zeros(len(train_normal)), np.ones(len(train_covid)), 2*np.ones(len(train_neumo))))
    val_files = valid_normal + valid_covid +valid_neumo
    val_labels = np.concatenate((np.zeros(len(valid_normal)), np.ones(len(valid_covid)), 2*np.ones(len(valid_neumo))))
    ...
    neumo_weights = len(train_labels) / np.sum(train_labels == 2*np.ones(np.shape(train_labels)))
    iters=0

    weights = np.array([[normal_weights, covid_weights, neumo_weights]])
    best_validation_accuracy = 0.0
    last_improvement = 0
    require_improvement = 1000
    prefix = ("Multi_deepv3_cls3"
    ...
    with tf.Graph().as_default():
        ...
        Accuracy_sum = tf.summary.merge([model.accuracy_sum, model.class1_acc_sum, model.class2_acc_sum, model.class3_acc_sum], name='Accuracy')
            ...
            for _, batchidx in mini_batches:
                if len(batchidx) == batch_size:
                    ...
                    train_out = sess.run([train_op, Loss_sum, Accuracy_sum, model.loss, model.avg_acc,
                                          model.avg_acc_class1, model.avg_acc_class2, model.avg_acc_class3],
                    ...
                    print("Iter:{}/{} time:{:4.4f} - loss: {:.4f} - acc:{:.4f} - normal_acc:{:.4f} "
                          "- covid_acc:{:.4f} - pneumo_acc:{:.4f}"
                          .format(iters, num_iter, time.time() - start_time,
                                  train_out[3], train_out[4], train_out[5], train_out[6], train_out[7]))
                    iters += 1
                    ...
                    if np.mod(iters, 500) == 5:
                        ...
                        for _, batch_idx in val_batches:
                            if len(batch_idx) == batch_size:
                                ...
                                val_out = sess.run([model.avg_acc, model.avg_acc_class1, model.avg_acc_class2, model.avg_acc_class3],
                            ...
                            avg_class3 += val_out[3] / (len(val_files) // batch_size)
                            print("validating...- acc:{:.4f} - normal_acc:{:.4f} - covid_acc:{:.4f} -neumo_acc:{:.4f} "
                                  .format(val_out[0], val_out[1], val_out[2], val_out[3]))

                        out_val.write("val_iter{} acc:{:.4f} normal_acc:{:.4f}  covid_acc:{:.4f} neumo_acc:{:.4f}"
                                      .format(iters, avg_acc, avg_class1, avg_class2, avg_class3))
                        out_val.write("\n")
                        ...
                        if avg_acc > best_validation_accuracy:
                            ...
                            print("Val_Iter:{} time:{:4.4f} - acc:{:.4f} - normal_acc:{:.4f} - covid_acc:{:.4f} - neumo_acc:{:.4f} -{}"
                                  .format(iters, time.time() - start_time,
                                          avg_acc, avg_class1, avg_class2, avg_class3, improved_str))
